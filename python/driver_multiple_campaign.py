...
import bid_landscape
config.dsp_l = bid_landscape.optimal_l
...
for cam in config.campaigns:
    ...
    cost_sum = 0.
    clk_sum = 0.
    fi = open(config.data_folder + cam + "/train.yzpc.txt", "r")
    for line in fi:
        s = line.strip().split("\t")
        clk = int(s[0])
        mp = int(s[1])
...
for cam in config.campaigns:
    cam_test_data[cam] = []
    fi = open(config.data_folder + cam + "/test.yzpc.txt", "r")
    for line in fi:
        s = line.strip().split("\t")
...
folder = "../results/bttt/stage-2/"
config.result_file = "%sarbitrage.results.multiple.%s.r-%.1f.txt" % (folder, camstr, config.cpc_payoff_ratio)
config.portfolio_file = "%sarbitrage.portfolio.multiple.%s.r-%.1f.txt" % (folder, camstr, config.cpc_payoff_ratio)
